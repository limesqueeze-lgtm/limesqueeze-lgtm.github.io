...
Hotel reviews in general are personal and may be posted anonymously. Ideally, the raw dataset at its source should be as clean and fair as possible. But this is not always true. Therefore, human intervention is still required in machine learning. 

Particularly for topic modelling, stopwords list can be manually edited and expanded to reduce noise. However, this process should be transparent and provided with the rationale for its inclusion or exclusion. For example, if the data engineer or pretrained model is not as familiar or aware of words and terms used by handicapped people or people who are neurodivergent, they may unknowingly add such words to the stopwords list and result in under-representation of this group of guests. This is to improve accountability and transparency of the model. Bad actors could also manipulate the data to create favourable findings or just for the sake of optimising the model. The responsibility lies on the data engineer to be ethical even if the results are contrary or go against what was expected. 

It is also important to ensure the accuracy of the dataset at its source as well. There have been news of AI being used to mass create reviews that are favourable towards a business in order to shape public perception. The opposite is also true where "review-bombing" is done to a rival company by creating alot of negative reviews. While this project utilised a dataset that was from kaggle which in turn was scrapped from the website booking.com, care must be taken to ensure that the reviews are not compromised. We should not totally rely on the source (ie. Booking.com or Kaggle) to ensure the fairness and accuracy of the dataset. Another consideration would be that negative reviews tend to be over-represented. This could lead to over-prioritising smaller issues that came from a vocal minority which goes against the business goal of being focused on the factors that actually matter. 

As mentioned, reviews sometimes come from a very personal and emotionally charged response to a good or bad experience. A guest who had a particularly bad stay may post a scathing review online about the hotel and end up including names of the staff or other sensitive information. At the data cleaning step, the dataset could be treated to remove names and avoid posting the raw dataset in reports. If that is unavoidable, anonymising the names should be implemented. Accuracy is also affected if sarcasm is used as LDA is not able to pick up on sarcasm. This may again lead to misinterpretation of themes. Perplexity and coherence alone would not be able to tell these and manual checking while cross-referencing with own business or domain knowledge of the subject may be required. 
